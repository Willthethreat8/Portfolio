...
const experiences: ExperienceItem[] = [
  {
    ...
    accent: "green",
    summary:
      "Conception et mise en œuvre de pipelines de données (ETL) et de modules d'Intelligence Artificielle pour l'optimisation de la prospection foncière, en collaboration avec Conjecto. Orchestration avec Airflow, transformation avec dbt, déploiement sur OVH Cloud.",
    achievements: [
      "ETL & Data Pipelines : conception et orchestration de pipelines ETL avec Airflow pour l'ingestion, la transformation et le chargement de données multi-sources (APIs Open Data : DVF, INSEE, Cadastre)",
      "Modélisation & Transformation : structuration et enrichissement des données CRM avec dbt, mise en place de tests de qualité et documentation automatisée des modèles",
      "Infrastructure Cloud : déploiement et administration de l'environnement data sur OVH Cloud (instances, stockage objet, bases managées)",
      "Intégration d'APIs : développement de connecteurs API REST pour l'extraction de données externes et l'alimentation automatique des pipelines",
      "NLP & Machine Learning : développement d'un module de détection de signaux faibles via crawling web, NLP et classification automatique de documents. Traitement des OAP (Orientations d'Aménagement et de Programmation) : récupération via API et analyse automatisée des documents avec Claude (LLM) pour structurer les données urbanistiques",
      "Géomatique (SIG) : traitements géospatiaux sur des données de zonage urbain (PLU) avec PostGIS pour l'aide à la décision foncière. Reverse geocoding à partir des coordonnées géographiques pour retrouver les adresses exactes des lots cadastraux",
    ],
    skills: ["Python", "Airflow", "dbt", "OVH Cloud", "API REST", "Claude (LLM)", "SQL", "PostGIS", "Reverse Geocoding", "NLP", "Machine Learning", "Pandas", "Scrapy", "Open Data"],
    links: {},
  },
  {
